...
@trace_decorator()
def oauth_register(provider):  # noqa: WPS210
    ...
    if user_service.filter_by(login=login, _first=True):
        return make_response(redirect('/login'))
    if user_service.filter_by(email=email, _first=True):
        return make_response(redirect('/login'))

    password = generate_password()
    user = user_service.create_user(user_params={'login': login,
                                                 'email': email,
                                                 'password': password,
                                                 'email_is_confirmed': True})
    user_service.generate_password_event(user_id=user.id)
    oauth_service.create_oauth(oauth_params={
        'user_id': user.id,
...
@trace_decorator()
def oauth_login(provider):  # noqa: WPS210
    oauth_service = getattr(oauth_client, f'get_{provider}_oauth_client_service', None)
    if oauth_service is None:
        return make_response(redirect('/login'))
    oauth_service = oauth_service()
    ...
    oauth = oauth_service.get_oauth(sub=sub, oauth_type=getattr(OAuthEnum, provider))
